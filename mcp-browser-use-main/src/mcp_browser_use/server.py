...
@mcp.tool()
async def inspect_page(cache_clickable_elements_hashes:bool) -> str:
    """
    Args:
    cache_clickable_elements_hashes (bool): If True, cache the clickable elements hashes for the current state. This is used to calculate which elements are new to the llm (from last message) -> reduces token usage.
    ...
    state = await browser_context.get_state(cache_clickable_elements_hashes)
    state.screenshot = ""
    return f"{state}"
